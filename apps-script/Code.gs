...
const SHARED_KEY = 'minneapolis-ope'; // must match SHARED_KEY in app.js
...
const SHEETS = {
  ...
  dylan: { name: 'DylanClicker', headers: ['name', 'count', 'owned', 'updatedAt'] },
};
...
// One row per person (upserted by saveDylan): their current Dylan count plus a
// JSON blob of which upgrades they own, so an idle-game save fully round-trips.
function readDylan() {
  const rows = getSheet('dylan').getDataRange().getValues();
  return rows.slice(1)
    .filter(r => r[0])
    .map(r => ({
      name: r[0],
      count: Number(r[1]) || 0,
      owned: parseOwned(r[2]),
      updatedAt: normDate(r[3]),
    }));
}
...
// Upsert a person's Bob Dylan Clicker save: update their existing row in place
// (so there's exactly one row per person), or append a new one. Unlike the
// round-based score sheets, this is a running save, not an append log.
function saveDylan(data) {
  const name = (data.name || '').trim();
  const count = Number(data.count);
  if (!name) return jsonResponse({ error: 'Name is required' });
  if (!Number.isFinite(count)) return jsonResponse({ error: 'Count must be a number' });

  const owned = (data.owned && typeof data.owned === 'object') ? data.owned : {};
  const ownedStr = JSON.stringify(owned);
  const updatedAt = new Date().toISOString();

  const sheet = getSheet('dylan');
  const values = sheet.getDataRange().getValues();
  for (let r = 1; r < values.length; r++) {
    if (String(values[r][0]).toLowerCase() === name.toLowerCase()) {
      // Keep the originally-stored casing of the name.
      sheet.getRange(r + 1, 1, 1, 4).setValues([[values[r][0], count, ownedStr, updatedAt]]);
      log(`DYLAN_SAVE "${values[r][0]}" count ${count}`);
      return jsonResponse({ name: values[r][0], count, owned, updatedAt });
    }
  }
  sheet.appendRow([name, count, ownedStr, updatedAt]);
  log(`DYLAN_SAVE "${name}" count ${count}`);
  return jsonResponse({ name, count, owned, updatedAt });
}
